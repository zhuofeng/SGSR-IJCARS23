...
class ResidualDenseBlock3D(nn.Module):
    ...
    def __init__(self, mid_channels=64, growth_channels=32):
        ...
        for i in range(5):
            ...
            self.add_module(
                f'conv{i+1}',
                nn.Conv3d(mid_channels + i * growth_channels, out_channels, 3,
                          1, 1))
    ...
    def init_weights(self):
        """Init weights for ResidualDenseBlock3D.
...
class ResidualDenseBlock3D_AdaDM(nn.Module):
    """Residual Dense Block.
    ...
    def __init__(self, mid_channels=64, growth_channels=32):
        ...
        self.phi = nn.Conv3d(1, 1, 1, 1, 0, bias=True)
        self.init_weights()
    ...
    def forward(self, x):
        ...
        Returns:
            Tensor: Forward results.
        """
        s = torch.std(x, dim=[1,2,3,4], keepdim=True)
        x = self.norm1(x)
        x1 = self.lrelu(self.conv1(x))
        x2 = self.lrelu(self.conv2(torch.cat((x, x1), 1)))
        x3 = self.lrelu(self.conv3(torch.cat((x, x1, x2), 1)))
        x4 = self.lrelu(self.conv4(torch.cat((x, x1, x2, x3), 1)))
        x5 = self.conv5(torch.cat((x, x1, x2, x3, x4), 1))
        x5 = x5 * torch.exp(self.phi(torch.log(s)))
        # Emperically, we use 0.2 to scale the residual for better performance
        return x5 * 0.2 + x
...
class RRDB3D(nn.Module):
    ...
    def __init__(self, mid_channels, growth_channels=32):
        super().__init__()
        self.rdb1 = ResidualDenseBlock3D(mid_channels, growth_channels)
        self.rdb2 = ResidualDenseBlock3D(mid_channels, growth_channels)
        self.rdb3 = ResidualDenseBlock3D(mid_channels, growth_channels)
...
class RRDB3D_AdaDM(nn.Module):
    ...
    def __init__(self, mid_channels, growth_channels=32):
        ...
        self.norm1 = nn.BatchNorm3d(mid_channels)
        self.phi = nn.Conv3d(1, 1, 1, 1, 0, bias=True)
        self.rdb1 = ResidualDenseBlock3D_AdaDM(mid_channels, growth_channels)
        self.rdb2 = ResidualDenseBlock3D_AdaDM(mid_channels, growth_channels)
        self.rdb3 = ResidualDenseBlock3D_AdaDM(mid_channels, growth_channels)
    ...
    def forward(self, x):
        ...
        out = self.rdb2(out)
        out = self.rdb3(out)
        out = out * torch.exp(self.phi(torch.log(s)))
        # Emperically, we use 0.2 to scale the residual for better performance
        return out * 0.2 + x
